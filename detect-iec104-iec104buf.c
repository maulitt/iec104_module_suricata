...
int DetectIEC104Match (DetectEngineThreadCtx *det_ctx, Packet *p, const Signature *s, const SigMatchCtx *ctx)
{
    ...
    SCLogNotice("Message from Match, with love <3");
    // простые проверки пакета, тут по док-ции ещё можно на ip проверить
    if (PKT_IS_PSEUDOPKT(p)) {
        SCLogNotice("Some pseudopkt detected:(");
        return 0;
    }

    if (!PKT_IS_TCP(p)) {
        SCLogNotice("This is not TCP transport");
        return 0;
    }

    /* packet payload access -- (C)документация */
    if (*(p->payload) != 0x68) {
        SCLogNotice("This is not IEC104, start byte isn't 68");
        return 0;
    } else {
        ret = 1;
    }
...
void DetectIEC104IEC104bufRegister(void)
{
    ...
    sigmatch_table[DETECT_AL_IEC104_IEC104BUF].desc =
            "IEC104 content modifier to match on the iec104 buffers";
    ...
    // register mpm engines - these are called in the prefilter stage        я закомментила
        ...
        void *txv, const int list_id)
{
    ...
    if (buffer->inspect == NULL) {
        ...
        if (flow_flags & STREAM_TOSERVER) {
            ...
            return NULL; //* no buffer                        я закомментила эту функцию
        }
