...
int type_inspect(int dec_input) {
    int bin_array[10], n, i;
    n = dec_input;
    i = 0;
    if (n == 0) {
        bin_array[0] = 0;
        ...
        const uint8_t *input, uint32_t input_len, uint8_t *rdir)
{
	...
	if (input[0] == 104) {                        // 104 это decimal от hex=68 (проверка на стартовый байт короче)
            SCLogNotice("IT MATCHED URAAAAAAAAAA");
	    if (type_inspect(input[2]) == 0) SCLogNotice("It's I-type");
	    if (type_inspect(input[2]) == 1) {
		if (input[1] == 4) { SCLogNotice("It's S-type");}
		else { return ALPROTO_UNKNOWN; }
	    }
	    if (type_inspect(input[2]) == 3) {
		if (input[1] == 4) {SCLogNotice("It's U-type %d", input[2]);}
		else { return ALPROTO_UNKNOWN; }
	    };
    	    return ALPROTO_IEC104;
	} else {
            SCLogNotice("FUCKKK this %d", input[0]);
        ...
        const uint8_t *input, uint32_t input_len, uint8_t *rdir)
{
    ...
    if (input_len >= IEC104_MIN_FRAME_LEN) {
        ...
        if (input[0] == 104) {                        // 104 это decimal от hex=68 (проверка на стартовый байт короче)
            SCLogNotice("IT MATCHED URAAAAAAAAAA");
	    if (type_inspect(input[2]) == 0) SCLogNotice("It's I-type to client TO CLIENT");
            if (type_inspect(input[2]) == 1) {
                if (input[1] == 4) { SCLogNotice("It's S-type TO CLIENT");}
                else { return ALPROTO_UNKNOWN; }
            }
            if (type_inspect(input[2]) == 3) {
                if (input[1] == 4) {SCLogNotice("It's U-type TO CLIENT %d", input[2]);}
                else { return ALPROTO_UNKNOWN; }
            };
...
void RegisterIEC104Parsers(void)
{
    ...
    if (AppLayerProtoDetectConfProtoDetectionEnabled("tcp", proto_name)) {
        ...
        SCLogNotice("Protocol detector and parser disabled for IEC104.");
        return;
    }
